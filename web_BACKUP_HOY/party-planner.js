document.addEventListener('DOMContentLoaded', () => {
    ...
    const generatePDF = async () => {
        ...
        if (leadId) {
            try {
                // Phase 5 Intelligence Engine Prep: Map text cities to Geospatial Coordinates
                const inputCity = blueprintData['venue_city'] || blueprintData['location'] || 'Miami'; 
                const cities = {
                    "Key Largo": { lat: 25.0865, lon: -80.4473 },
                    "Miami": { lat: 25.7617, lon: -80.1918 },
                    "Orlando": { lat: 28.5383, lon: -81.3792 },
                    "New York": { lat: 40.7128, lon: -74.0060 }
                };
                
                // Fallback to Miami if the city is not explicitly in the dictionary
                const coords = cities[inputCity] || cities["Miami"];

                // Extract core fields that MUST be searchable/queried by the SaaS Engine at the top level of the table
                ...
                else console.log('Lead blueprint + Geospatial Coordinates saved to Supabase');
            } catch (e) {
