        ...
        <div style="animation: fadeIn 0.4s ease-out; color: #fff; font-family: 'Inter', sans-serif;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; background: rgba(0,0,0,0.2); backdrop-filter: blur(20px); padding: 25px; border-radius: 24px; border: 1px solid rgba(255,255,255,0.05);">
                ...
                <div>
                    ...
                    <div style="font-size: 18px; font-weight: 700; color: #fff;">${data.name || "N/A"}</div>
                </div>
...
function applyWeatherScene(scene, weatherMain) {
    ...
    if (weatherContainer) {
        ...
        weatherContainer.classList.remove('scene-clear-day', 'scene-sunset', 'scene-clear-night', 'scene-cloudy-day', 'scene-cloudy-night', 'scene-rain', 'scene-rain-night', 'scene-rain-day', 'scene-storm', 'scene-storm-day', 'scene-snow');
    ...
    // Dynamic Lightning Orchestration
    if (scene === 'storm') {
...
window.startDynamicLightning = function() {
    ...
    // Astral is z-index 2 (Behind Clouds which are 3 & 4)
    const container = document.getElementById('weather-layer-astral');
    if (!container) return;
    
    // Purge old nodes just in case
    container.querySelectorAll('.dynamic-lightning-svg').forEach(el => el.remove());
    
    function createLightningPath(startX, startY, endY, branches, isMain, hBias = 0) {
        let path = `M ${startX} ${startY}`;
        let currX = startX;
        let currY = startY;
        const segmentCount = isMain ? 12 + Math.floor(Math.random() * 8) : 5 + Math.floor(Math.random() * 4);
        const yStep = (endY - startY) / segmentCount;

        for (let i = 0; i < segmentCount; i++) {
            currY += yStep + (Math.random() * 20 - 10);
            
            // Much wider Jitter to prevent straight beams
            let xJitter = (Math.random() * 80 - 40);
            if (!isMain) {
                // Secondary branches push aggressively outward
                xJitter = hBias + (Math.random() * 60 - 30);
            }
            ...
            if (branches > 0 && Math.random() > (isMain ? 0.6 : 0.8)) {
                const branchBias = (Math.random() > 0.5 ? 80 : -80); // push left or right
                // Short, aggressive side branches
                const subBranchEndY = currY + (Math.random() * 150 + 50);
                path += createLightningPath(currX, currY, subBranchEndY, branches - 1, false, branchBias);
    ...
    function strike() {
        ...
        svg.style.zIndex = "2"; // Ensure it stays behind clouds
        // Add random slight rotation and scale for variety
        const scaleX = Math.random() > 0.5 ? 1 : -1;
        svg.style.transform = `scaleX(${scaleX})`;

        // Lightning originates from upper cloud bank (Y: 15% to 30% matching new ceiling)
        // We use viewport-relative coordinates for the SVG path
        const rect = container.getBoundingClientRect();
        const startX = Math.random() * rect.width; 
        const startY = rect.height * (0.15 + Math.random() * 0.15); // Originates from top clouds
        const endY = rect.height * (0.6 + Math.random() * 0.3);

        const pathData = createLightningPath(startX, startY, endY, 3, true);

        // Glow Layer (Thick, blurred, lower opacity)
        const glowPath = document.createElementNS(svgNS, "path");
        glowPath.setAttribute("d", pathData);
        glowPath.setAttribute("stroke", "rgba(180, 200, 255, 0.5)");
        glowPath.setAttribute("stroke-width", "12");
        glowPath.setAttribute("fill", "none");
        glowPath.style.filter = "blur(8px)";

        // Core Layer (Thin, sharp, bright white)
        ...
        corePath.style.filter = "blur(1px)";
