// SSOT: Única Fuente de Verdad para Colores de Agenda (Perfil + Dashboard)
window.CALENDAR_THEMES = {
    gig:       { label: 'Evento', color: '#00ff88', css: 'state-gig' },
    resident:  { label: 'Residencia', color: '#c5a059', css: 'state-resident' },
    vacation:  { label: 'Vacaciones', color: '#5078ff', css: 'state-vacation' },
    locked:    { label: 'Cancelado', color: '#ff5555', css: 'state-locked' },
    available: { label: 'Disponible', color: 'transparent', css: 'state-available', ring: true },
    today:     { label: 'Hoy', color: '#ffffff', css: 'state-today' }
};

// FASE 3: CSS Mínimo Dinámico Idempotente
let themeStyle = document.getElementById('dynamic-calendar-themes');
if (!themeStyle) {
    themeStyle = document.createElement('style');
    themeStyle.id = 'dynamic-calendar-themes';
    document.head.appendChild(themeStyle);
}
themeStyle.innerHTML = `
    #calendar-master .state-gig { background: ${window.CALENDAR_THEMES.gig.color}26 !important; border: 1px solid ${window.CALENDAR_THEMES.gig.color}66 !important; }
    #calendar-master .state-gig:hover .fc-daygrid-day-frame { background: ${window.CALENDAR_THEMES.gig.color}80 !important; }
    #calendar-master .state-resident, #calendar-master .state-resident .fc-daygrid-day-bg, #calendar-master .state-resident .fc-daygrid-day-frame { background: ${window.CALENDAR_THEMES.resident.color}26 !important; border: 1px solid ${window.CALENDAR_THEMES.resident.color}66 !important; }
    #calendar-master .state-resident:hover .fc-daygrid-day-frame { background: ${window.CALENDAR_THEMES.resident.color}80 !important; }
    #calendar-master .state-vacation, #calendar-master .state-vacation .fc-daygrid-day-frame { background: ${window.CALENDAR_THEMES.vacation.color}0D !important; opacity: 0.8 !important; }
    #calendar-master .state-locked { background: ${window.CALENDAR_THEMES.locked.color}14 !important; border: 1px solid ${window.CALENDAR_THEMES.locked.color}4D !important; }
`;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const isFullPro = isPro && isNotExpired;
    ...
    if (referralInput && qrContainer) {
        const referralCode = profile.referral_code || profile.member_id || `MDJ-${user.id.substring(0, 5).toUpperCase()}`;
    ...
    // Solo #calendar-master: el dashboard usa AgendaEngine en #agenda-calendar-master (un solo motor FullCalendar).
        ...
        if (profile.availability_schedule && typeof profile.availability_schedule === 'object') {
            ...
            const recurringEvents = recurringDays.map(day => ({
                ...
                extendedProps: {
                    ...
                    status: 'RESIDENT',
                    start_time: '22:00', // Horario default de Residencia
                    end_time: '03:00',
                    buffer_time: 60
                },
                backgroundColor: window.CALENDAR_THEMES.resident.color,
                borderColor: window.CALENDAR_THEMES.resident.color
            }));
...
window.initEventWeatherCalendar = async function (assignedEvents = []) {
    const calendarEl =
        document.getElementById('calendar-master') || document.getElementById('agenda-calendar-master');
    ...
    try {
        ...
        if (session) {
            ...
            // Iniciar Realtime de Notas Operativas Fase 1
            if (typeof window.setupEventNotesRealtime === 'function') {
    ...
    const vacEvents = vacationDates.map(date => {
        ...
        return {
            ...
            start: date,
            allDay: true,
            extendedProps: { eventId: `vac-${date}`, type: DAY_STATE.VACATION }
        };
    });
    ...
    const calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventContent: function (arg) {
            ...
            if (type === DAY_STATE.EVENT) {
                ...
                dot.style.backgroundColor = window.CALENDAR_THEMES.resident.color; dot.style.color = window.CALENDAR_THEMES.resident.color;
            } else if (type === DAY_STATE.VACATION) {
                dot.style.backgroundColor = window.CALENDAR_THEMES.vacation.color; dot.style.color = window.CALENDAR_THEMES.vacation.color;
            } else {
        ...
        eventClick: function (info) {
            if (window.calendarSelectionTimer) clearTimeout(window.calendarSelectionTimer);
        ...
        dateClick: function (info) {
            ...
            if (window.handleEventWeather) {
                matchingEvent = info.view.calendar.getEvents().find(e => {
                    const eStart = e.startStr || (e.start ? e.start.toISOString().split('T')[0] : '');
                    return eStart.startsWith(info.dateStr);
                });
                window.handleEventWeather(matchingEvent || info.dateStr);
            }

            if (!matchingEvent) {
                window.calendarSelectionTimer = setTimeout(() => {
                    document.querySelectorAll('.fc-daygrid-day').forEach(d => d.classList.remove('fc-day-selected'));
                    if (window.handleEventWeather) {
                        ...
                        const todayEv = info.view.calendar.getEvents().find(e => {
                            ...
                            return eS.startsWith(localToday);
                        });
                        window.handleEventWeather(todayEv || new Date());
                    }
                }, 10000);
            }
        },

        // PHASE 7: CHROME STABILIZATION HOOK
        // When FullCalendar finishes rendering the grid, alert the Intelligence Engine
        datesSet: function (info) {
            console.log("📅 [FullCalendar] Grid rendered. Dispatching 'djCalendarRendered' event.");
            const event = new CustomEvent('djCalendarRendered', { detail: { view: info.view } });
            document.dispatchEvent(event);

            // AUTO-LOAD TODAY'S EVENT ON INITIALIZATION
            if (!window.initialCalendarLoaded) {
                ...
                }, 400); // Slight delay for rendering
            }
        }
    });
...
window.showDashEventDetails = function (dateStr, isRecurring, dayData, dayNum) {
    ...
    if (dayData && dayData.events && dayData.events.length > 0) {
        ...
        themeColor = isCancelled ? window.CALENDAR_THEMES.locked.color : window.CALENDAR_THEMES.gig.color;
        ...
        themeColor = window.CALENDAR_THEMES.resident.color;
        ...
        themeColor = window.CALENDAR_THEMES.vacation.color;
        html = `
...
window.showEventWeatherDetails = async function (eventOrDate) {
    if (window.handleEventWeather) {
        return window.handleEventWeather(eventOrDate);
    } else {
        console.error("MDJPRO Live Weather Engine (handleEventWeather) not found.");
    }
};

// ── FLUJO ANALYTICS (MDJPRO PREMIUM) ───────────────────────
// window.loadFlowData mock removed (Finalization Phase).
// Handled strictly by flow-handler.js Engine to prevent fake data overriding.
