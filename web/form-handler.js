...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (talentInput) {
        talentInput.addEventListener('input', debounce(async (e) => {
            ...
            const { data, error } = await dbClient
                .from('dj_profiles')
                .select('dj_name, availability, status, available')
                .ilike('dj_name', `%${name}%`)
                .eq('available', true)
                .limit(1)
                .single();
    ...
    // ── Eye Toggle (Mostrar/Ocultar Password en forms genéricos) ─────────────
    document.querySelectorAll('.hero-pwd-toggle, .pwd-toggle').forEach(btn => {
        // Only attach if it wasn't attached by login.html already
        if (!btn.hasAttribute('data-pwd-listening')) {
            btn.setAttribute('data-pwd-listening', 'true');
...
async function checkTalentAvailability() {
    ...
    const { data, error } = await db2
        .from('dj_profiles')
        .select('dj_name, availability, available')
        .ilike('dj_name', `%${name}%`)
        .eq('available', true)
        .limit(1)
        .single();
