...
## 2. LEY DE SEGURIDAD Y ROLES (CIERRE DE BRECHA) — INCLUYE «ZONA ROJA»
- La función `public.is_staff(uid)` (y `is_staff_management` donde aplique) en Postgres es la **única** fuente de verdad operativa para staff; **`public.dj_profiles.role`** es la fila que esas funciones leen.
...
- **PROHIBIDO MEZCLAR PERFILES (INCIDENT-001):** El rol `owner` es Staff. El botón "STAFF" en la navegación principal DEBE redirigir a `./admin-dashboard.html#staff` para el owner, NUNCA a `account-profile.html`.
- **PROHIBIDO BLOQUEAR EL AUTH GATE:** En `admin-dashboard.html`, nunca inyectar llamadas a funciones pesadas (ej. `loadMdjproDownloadsCatalog`) antes de ejecutar `document.documentElement.classList.remove('mdj-admin-gate-pending')`. Esto causa una pantalla negra irreversible.
...
# LOCKED FILES ARCHITECTURE

The following files are strictly LOCKED for layout, structure, and text modifications.
As an AI agent, you must obey these rules or you will corrupt the frontend architecture.

LOCKED FILES:
web/index.html
web/styles.css
web/dj-knowledge.html
web/courses.html
web/jobs.html
web/dj-profile.html
...
DO NOT:
- restructure DOM
- wrap containers in new divs
- remove hardcoded text
- replace literal text with empty data-i18n tags
- modify navigation layout

EDIT ONLY targeted blocks and specific functionalities as strictly instructed by the user. "Trust, but verify" before touching these files.

# MANDATORY — selected zone only (no drive-by edits)
...
The owner navigation bar on the DJ public profile is **finished and frozen**:
...
- Query param: **`mdj_nav=profile`**. Artist links from profile/dashboard/academia strips to Jobs / Shop / DJ Tools / Agenda / etc. **must** preserve it. **`index.html` and public entry points do not** add it (normal site bar).
- **Home** in the artist strip is **`./index.html` without** `mdj_nav` (returns to public site nav).
...
- **Artistic / stage / brand** (e.g. **DJMago305**) — public profile, promos, social, stage. **Not** for formal documents.
- **Legal full name** (e.g. **Gerardo A Valle**) — **certificates**, **courses** (instructor attribution), **contracts**, invoices, and any **official** copy.
- UI and data should keep both distinct (`full_name` vs `stage_name` / display DJ brand). Do not use the stage handle as the sole name on certificates or legal surfaces.
...
- The site is **bilingual: Spanish and English** across UI copy where practical.
- **Official / canonical language is English (`en`).** Spanish (`es`) is the secondary language.
- Prefer **English** for legal, technical, product, and brand strings when in doubt.
- If a Spanish string is awkward, unclear, or untranslatable, **keep the English** (do not force Spanish).
- New `[data-i18n]` keys: add **English first** in `translations.js`, then Spanish when available.
- Runtime behavior: `i18n.js` falls back to the other locale when a key is missing in the active language.

# Workflow control (regresiones, auth, deploy)
...
- Los archivos vivos que lee el servidor son únicamente: 'services.html', 'events.html', 'rentals.html' dentro de './web/'.
- Tienes estrictamente prohibido crear copias de seguridad (.bak, .old, .copy) o archivos espejo en la raíz.
- Ignora cualquier otra terminal o puerto que no sea el archivo físico real conectado en el puerto 8080.
## 5. LEY DE MINIMALISMO Y CÓDIGO ESTRICTO (CERO INICIATIVAS NO SOLICITADAS)
- **CERO AGREGADOS:** Si un elemento (botón, tarjeta, sección, enlace) no fue explícitamente solicitado por el Capitán en el ticket, **NO SE AGREGA**.
- **MENOS ES MÁS:** El código debe mantenerse lo más limpio y minimalista posible. No se deben crear "accesos directos", "mejoras visuales no pedidas" ni "cables innecesarios".
- **OBEDIENCIA MILITAR:** La IA debe ejecutar **exclusivamente** el trabajo explícitamente explicado. Toda iniciativa propia que agregue elementos a la UI sin orden directa será considerada una violación del protocolo.
