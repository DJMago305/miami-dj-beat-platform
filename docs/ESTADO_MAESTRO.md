...
- [2026-08-25] **CIERRE FINAL — PR #268 fusionado a `main`, rama `feature/contracts-w9-engine-integration` eliminada.** Última sub-sesión sobre la misma rama, en tres tandas:
  - **FASE 1/2/3 (commit `d6b9275`)**: estandarización financiera — `depositDueDate`/`overtimeRate` sumados a `live_band`, `solo_artist`, `subcontractor_show` (ya existían en `dj`/`private_event` desde antes); fila "Montaje / Soundcheck" reflejada en el resumen `.kv` de las 5 plantillas con esos campos. Especialización de eventos: mecanismo genérico y reutilizable de campos condicionales (`showIf`+`refreshFormOnChange`) — Quinceañera en `private_event` (nombre + firma de padre/tutor como firmante adicional + cláusula de protocolo Vals/Brindis/Hora Loca) y switch "Presentación en Club/Festival" en `dj` (rider técnico de cabina Pioneer CDJ/DJM + cláusula de cortesías de acceso técnico).
  - **Verificación del render**: las 5 plantillas probadas inyectando `postMessage({type:'mdjb:open-template'})` — mismo canal real que usa `staff.html` — contra `contracts-engine.html` servido localmente; sin errores de consola, sin desborde horizontal en `#paper`, toggles Quinceañera/Club confirmados idempotentes en ambas direcciones.
  - **Certificación RBAC del contenedor real**: se confirmó por código que el puente `staff.html`↔`contracts-engine.html` (acordeón "⚖️ Bóveda Legal", 9 `data-legal-tpl`) mapea 1:1 con los 9 ids de `TEMPLATES`. La sesión de prueba disponible era de artista, no de staff/owner — el gate de `staff.html` (`dj_profiles.role` contra `STAFF_ROLES`) la redirigió correctamente a `dj-dashboard.html`; no se fabricó ni se intentó saltar esa sesión. El PO aceptó esta verificación como suficiente ("El control de acceso RBAC está operando correctamente y la verificación del puente postMessage es suficiente") y dio la aprobación final.
  - **PR #268 abierto y fusionado por el PO manualmente** (directiva de flujo vigente para esta rama: "los PRs y merges a main los realiza el PO"), merge commit `962daa6`. Confirmado `git merge-base --is-ancestor` antes de borrar: rama totalmente contenida en `main`. Rama local y remota `feature/contracts-w9-engine-integration` eliminadas tras el merge.
  - **Motor de Contratos y W-9 — saga cerrada.** Pendientes que sobreviven a este cierre (sin bloquear producción): datos de `financial_venues` sin `address`/`contact_*`; verificación con sesión real de DJ del widget de compliance y "Bóveda Legal" en la Cabina DJ; decisión de producto de "Fénix AI" para artistas.
