...
3. **Los 4 motores — estado real, no aspiracional:**
   ...
   - **Recomendador Armónico Camelot** (compatibilidad de tonalidades + curvas de BPM) — **CONSTRUIDO Y VERIFICADO** (corrección 2026-08-30, posterior al registro original de esta Constitución: commit `e83fb3a`, `web/js/smartcrate-harmonic-engine.js`). Diccionario completo de conversión de notación tradicional→Camelot (24 claves + enarmónicos), 4 reglas de compatibilidad armónica, y filtro de BPM por tipo de evento. Verificado con Node contra los 13 ejemplos de conversión y 7 casos de compatibilidad dados por el PO, incluyendo el wraparound circular 12↔1. Ya integrado y renderizando en `staff.html#view-smartcrate` contra los 369 tracks reales de la instantánea de prueba. Esta entrada quedó desactualizada un rato el mismo día — la brecha original (Serato da notación tradicional, no Camelot) fue exactamente lo que motivó construir este motor.
