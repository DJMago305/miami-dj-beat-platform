// MDJPRO desktop heartbeat — thin Edge wrapper → mdjpro_heartbeat (service_role RPC).
// Deploy: supabase functions deploy mdj-heartbeat --no-verify-jwt
...
function auditLog(requestId: string, status: string, reason: string): void {
  console.log(
    `[MDJPRO] mdj-heartbeat | request_id=${requestId} | status=${status} | reason=${reason}`,
  );
}
...
serve(async (req) => {
  ...
  const leaseId = optionalText(body.lease_id);
  const deviceFingerprint = optionalText(body.device_fingerprint);

  if (!leaseId) {
    auditLog(requestId, "rejected", "lease_id_required");
    return jsonResponse({ ok: false, reason: "lease_id_required" }, 400, corsHeaders);
  }
  ...
  const { data, error } = await supabase.rpc("mdjpro_heartbeat", {
    p_lease_id: leaseId,
  ...
  if (result.ok === true) {
    auditLog(requestId, "ok", "heartbeat_ok");
