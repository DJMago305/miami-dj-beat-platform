-- ═══════════════════════════════════════════════════════════════════════════
-- 🔴🔴🔴  PRODUCCIÓN  🔴🔴🔴   proyecto: mdjb  ·  ref: hkuvuqupbxwkiykxvqdr
--
-- ESTE es el archivo para PRODUCCIÓN. Es IDÉNTICO, línea por línea, al que ya
-- validaste en PRUEBA (mdjb-ensayo) el 2026-08-17. NO cambió ninguna lógica:
-- solo esta cabecera. Esa es la garantía de "cero desvíos": lo que probaste = esto.
--
-- ANTES DE CORRER, confirma arriba a la izquierda en Supabase que el proyecto
-- seleccionado dice  mdjb  (ref hkuvuqupbxwkiykxvqdr), NO mdjb-ensayo.
--
...
--   • NOTA prod: la sección 0 deja a TODOS los usuarios reales en plan='free' (correcto por ahora;
--     Gmail/Calendar son de bloques posteriores). Al conectar Stripe: update dj_profiles set plan='pro'…
...
-- select * from public.plan_entitlements;   -- debe dar 3 filas: free / pro / super
-- select public.fenix_can('00000000-0000-0000-0000-000000000000'::uuid, 'gmail.read');  -- debe dar false
