...
--
--  ANTES DE PULSAR RUN: mira la URL del navegador y confirma que dice
--  hkuvuqupbxwkiykxvqdr. El proyecto de produccion se muestra con un nombre
--  enganoso; el ref de la URL es lo unico fiable.
--
--  Este guion hace TODO de una vez y en este orden:
--    1. Crea el esquema del medidor (tablas, funciones, RLS).
--    2. Da de alta la bolsa de owner y staff, guiado por dj_profiles.role.
--    3. Muestra a quien le quedo bolsa, para que lo verifiques.
--
--  Es idempotente: puede ejecutarse mas de una vez sin romper nada.
--  Validado de punta a punta en mdjb-ensayo el 2026-08-20.
--
...
-- ═══════════════════════════════════════════════════════════════════════════
--  PASO 2 · ALTA DE OWNER Y STAFF  —  parte obligatoria de la migracion
--
--  Sin fila de cuota, la Edge Function responde 'no_quota' y bloquea la voz.
--  Antes de este cambio el staff tenia voz sin que nadie preguntara por saldo;
--  desde ahora necesita bolsa. Si este bloque no corre, te quedas sin voz.
--
--  Sin marcadores que reemplazar: se guia por el rol en dj_profiles.
--  'distinct on' evita el fallo de Postgres si un usuario tuviera dos perfiles.
-- ═══════════════════════════════════════════════════════════════════════════

insert into public.elixis_voice_quotas (user_id, plan, is_unlimited, safety_cap_seconds)
select distinct on (p.user_id)
       p.user_id,
       case when lower(coalesce(p.role,'')) = 'owner' then 'owner' else 'staff' end,
       true,
       -- Owner 50 h/mes, resto del staff 10 h/mes. Son fusibles anti-bucle,
       -- no facturacion: el staff no se bloquea por saldo, solo por el tope.
       case when lower(coalesce(p.role,'')) = 'owner' then 180000 else 36000 end
  from public.dj_profiles p
 where lower(coalesce(p.role,'')) in ('owner','admin','manager','seller')
 order by p.user_id, p.role
on conflict (user_id) do update
   set plan               = excluded.plan,
       is_unlimited       = excluded.is_unlimited,
       safety_cap_seconds = excluded.safety_cap_seconds,
       updated_at         = now();


-- ═══════════════════════════════════════════════════════════════════════════
--  PASO 3 · COMPROBACION
--  TIENE que aparecer tu cuenta de owner. Si sale vacio, NO despliegues:
--  significa que no hay ningun perfil con rol owner/staff y te quedarias
--  sin voz. Avisa antes de seguir.
-- ═══════════════════════════════════════════════════════════════════════════

select q.plan,
       q.is_unlimited,
       q.safety_cap_seconds,
       round(q.safety_cap_seconds / 3600.0, 1) as horas_fusible,
       p.role,
       coalesce(p.stage_name, p.dj_name, p.full_name) as nombre
  from public.elixis_voice_quotas q
  join public.dj_profiles p on p.user_id = q.user_id
 order by case when q.plan = 'owner' then 0 else 1 end, p.role;


-- ═══════════════════════════════════════════════════════════════════════════
--  LOS ARTISTAS NO ENTRAN TODAVIA, a proposito.
--  El rol 'artist' ya esta habilitado en la Edge Function, pero sin fila de
--  cuota reciben 'no_quota' y caen a texto. Las bolsas del plan PRO se dan de
--  alta una a una cuando empiece el cobro:
--
--  insert into public.elixis_voice_quotas
--         (user_id, plan, flagship_monthly_seconds, mini_monthly_seconds)
--  select p.user_id, 'pro', 10800, 18000
--    from public.dj_profiles p
--   where p.user_id = 'PEGA_AQUI_EL_UUID_DEL_ARTISTA'::uuid
--  on conflict (user_id) do update
--     set plan = 'pro', flagship_monthly_seconds = 10800, mini_monthly_seconds = 18000;
-- ═══════════════════════════════════════════════════════════════════════════
