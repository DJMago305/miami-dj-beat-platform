-- Public roster / fan profile read model (anon + authenticated fans).
-- Builds the view from columns that actually exist in prod (skips missing ones).
...
COMMENT ON VIEW public.public_dj_profiles IS
  'Safe public read of dj_profiles for roster/find-dj/fan profile URLs. security_invoker=false bypasses base-table RLS.';
