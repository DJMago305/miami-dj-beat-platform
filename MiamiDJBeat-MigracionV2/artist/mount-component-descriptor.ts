/** MOD-011 Artist Dashboard — MOD-009 descriptor mount — TICKET-MOD-011-ARTIST-DASHBOARD-MVP-001 */
...
export function mountComponentDescriptor(descriptor: MdjComponentDescriptor): HTMLElement {
  const root = document.createElement(
    descriptor.componentId === 'KpiCard' || descriptor.componentId === 'ModuleCard'
      ...
      : descriptor.componentId === 'SectionHeader'
        ? 'h2'
        : descriptor.componentId === 'ProfileCard'
          ? 'div'
          : 'section',
  );
  ...
  switch (descriptor.componentId) {
    ...
    case 'ProfileCard': {
      root.classList.add('mdj-artist-profile-card');

      const name = document.createElement('p');
      name.className = descriptor.slots.nameClassName ?? 'mdj-shell-profile__name';
      name.textContent = descriptor.slots.name;

      const role = document.createElement('p');
      role.className = descriptor.slots.roleClassName ?? 'mdj-shell-profile__role';
      role.textContent = descriptor.slots.role;

      const meta = document.createElement('p');
      meta.className = descriptor.slots.metaClassName ?? 'mdj-shell-profile__meta';
      meta.textContent = descriptor.slots.meta;

      root.append(name, role, meta);
      break;
    }
    case 'KpiCard': {
